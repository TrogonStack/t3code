...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const { workEntry, workspaceRoot, isExpandedToolGroupEntry, displayLabel } = props;
  const { threadRef, onImageExpand, activeThreadEnvironmentId, timestampFormat } =
    use(TimelineRowCtx);
  ...
  const subagentShell =
    subagent?.threadId != null
      ? threadShells.find(
          (shell) =>
            shell.environmentId === activeThreadEnvironmentId && shell.id === subagent.threadId,
        )
      : undefined;
  const subagentLabel = subagentShell?.title ?? subagent?.title ?? null;
  // A failed call reads as failed whatever spawned it, so the bot icon only stands in for the
  // ordinary case.
  const entryIconName =
  ...
  const entryToolIcon =
    showWarningIndicator || showDestructiveRowStyle || subagent !== null
      ? undefined
  ...
  const openSubagentThread = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    void navigate({
      ...
      params: buildThreadRouteParams(
        scopeThreadRef(activeThreadEnvironmentId, ThreadId.make(subagent.threadId)),
      ),
    });
  };
              ...
              {subagent !== null && subagentLabel !== null ? (
                subagent.threadId !== null ? (
                  <button
                    ...
                    className="min-w-0 shrink-[2] cursor-pointer truncate rounded-full border border-border/60 px-1.5 text-2xs leading-4 text-muted-foreground transition-colors hover:border-border hover:text-foreground"
                  >
                    {subagentLabel}
                  </button>
                ) : (
                  <span className="min-w-0 shrink-[2] truncate rounded-full border border-border/40 px-1.5 text-2xs leading-4 text-muted-foreground/70">
                    {subagentLabel}
                  </span>
                )
              ) : null}
