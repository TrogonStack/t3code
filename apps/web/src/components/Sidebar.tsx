...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  thread: SidebarThreadSummary;
  // Nesting depth under the orchestrator that spawned this thread; 0 for
  // top-level threads.
    ...
    <li
      ...
      style={{
        ...(sortable
          ...
          : {}),
        ...(subagentDepth > 0 ? { paddingLeft: `${subagentDepth * 0.875}rem` } : {}),
      }}
      {...(sortable?.listeners ?? {})}
      className={cn(
...
export default function Sidebar() {
  ...
  // The sidebar can fork any thread, not just the active one, so it hosts its
  // own fork dialog rather than reusing ChatView's.
  ...
  const {
    ...
    // Subagent children sit directly under the orchestrator that spawned
    // them; only the inbox nests, since the parked sections are ordered by
    // wake time and settle time rather than by tree shape.
    const { ordered: orderedActive, subagentDepthByThread } = orderThreadsWithSubagents(
      ...
      void (async () => {
        ...
        switch (clicked.value) {
          ...
          case "mark-unread":
            ...
            return;
          case "fork":
            setForkThreadTarget({ threadRef, title: thread.title });
            return;
          case "copy-path":
              ...
              <ul ref={attachListAutoAnimateRef} role="list" className="flex flex-col gap-px">
                {(() => {
                  const renderThreadRow = (
                    ...
                    return (
                      <SidebarThreadRow
                        // Depth in the subagent tree; only inbox rows nest.
                        subagentDepth={
                          section === "active" ? (subagentDepthByThread.get(thread) ?? 0) : 0
                        }
