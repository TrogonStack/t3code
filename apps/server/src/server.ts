...
import { ProviderSecretResolverLive } from "./provider/Layers/ProviderSecretResolverLive.ts";
...
const RuntimeCoreDependenciesWithoutThreadBootstrapLive = ReactorLayerLive.pipe(
  ...
  // Split into a second `.pipe()` call: a single call tops out at 20
  // `Layer.provideMerge` arguments per Effect's `.pipe()` overloads.
).pipe(
  ...
  // Resolves `op://` environment values for both halves above: the drivers
  // read secrets while building an instance, and `ProviderRegistryLive`
  // drops the cached values when the user refreshes.
