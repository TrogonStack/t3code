...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const capabilitiesForInstance = (providerInstanceId: ProviderInstanceId) =>
    registry.getInstanceInfo(providerInstanceId).pipe(
      Effect.map((info): ReadonlySet<McpInvocationContext.McpCapability> =>
        info.driverKind === "claudeAgent" ? new Set(["preview", "threads"]) : new Set(["preview"]),
      ),
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      // Subagent threads ride on the same credential, so the capability set has to be
      // decided before it is minted rather than added to a token already issued.
      const capabilities = yield* capabilitiesForInstance(providerInstanceId);
