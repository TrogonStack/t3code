...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  /** Only Claude orchestrators can spawn subagent threads; other drivers never get the capability. */
  const supportsSubagentThreads = (providerInstanceId: ProviderInstanceId) =>
    registry.getInstanceInfo(providerInstanceId).pipe(
      Effect.map((info) => info.driverKind === "claudeAgent"),
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      // Subagent threads ride on the same credential, so the capability has to be
      // decided before it is minted rather than added to a token already issued.
      if (yield* supportsSubagentThreads(providerInstanceId)) capabilities.add("threads");
