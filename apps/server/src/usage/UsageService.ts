  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    for (const driver of ["claudeAgent", "codex", "grok"] as const) {
      ...
      for (const instance of instances) {
        // Usage reads only locate provider home directories, so the instance's
        // configured values are used as written: nothing here resolves a secret
        // reference, and so nothing here can report one as unresolved.
        const environment = mergeProviderInstanceEnvironment(
          { variables: instance.environment, unresolved: [] },
          hostEnvironment,
        );
