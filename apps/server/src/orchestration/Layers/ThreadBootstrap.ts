...
/**
 * What the caller knows about who asked. A bootstrap is several commands, and every one of them
 * was caused by the same request, so the origin travels with all of them rather than only the
 * turn start.
 */
type DispatchOptions = Parameters<ThreadBootstrapShape["dispatchBootstrapTurnStart"]>[1];
...
const makeThreadBootstrap = Effect.gen(function* () {
  ...
  const recordWorktreeSetup = (snapshot: WorktreeSetupSnapshot, options: DispatchOptions) =>
  ...
  const dispatchBootstrapTurnStart: ThreadBootstrapShape["dispatchBootstrapTurnStart"] = (
    command,
    options,
  ) =>
    Effect.gen(function* () {
      ...
      const bootstrapProgram = Effect.gen(function* () {
        ...
        const started = yield* Effect.uninterruptible(
          orchestrationEngine.dispatch(finalTurnStartCommand, options),
        );
      ...
      // caller only waits on the detached fiber; a user cancel interrupts
  ...
  } satisfies ThreadBootstrapShape;
});

export const ThreadBootstrapLive = Layer.effect(ThreadBootstrapService, makeThreadBootstrap);
