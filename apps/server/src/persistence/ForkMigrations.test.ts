...
// Derived from the registry so adding a fork migration does not mean editing
// every ledger assertion; what is under test is that the whole fork chain
// lands here rather than in the shared ledger.
const EXPECTED_FORK_LEDGER = forkMigrationEntries.map(([id, name]) => ({
  migration_id: id,
  name,
}));
...
layer("fresh install", (it) => {
  it.effect("runs the fork chain on its own ledger without touching shared numbering", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(forkLedger, EXPECTED_FORK_LEDGER);
