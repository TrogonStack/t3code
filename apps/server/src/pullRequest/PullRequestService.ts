...
export const make = Effect.gen(function* () {
  ...
  const bumpFilesViewedEpoch = (ref: PullRequestRef) => {
    const scope = refScope(ref);
    if (!filesViewedEpochs.has(scope) && filesViewedEpochs.size >= REF_EPOCH_CAPACITY) {
      const oldest = filesViewedEpochs.keys().next().value;
      if (oldest !== undefined) filesViewedEpochs.delete(oldest);
    }
    filesViewedEpochs.set(scope, ++epochCounter);
  };
