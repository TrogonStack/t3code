...
## Anthropic rejected a token

You see this on a provider whose environment variables set `CLAUDE_CODE_OAUTH_TOKEN`. T3 Code asked
Anthropic about that token and Anthropic refused it, so the provider is marked not authenticated
before you spend a message finding out.
...
A value that is not an Anthropic credential never causes this message: T3 Code only asks Anthropic
about something that looks like one, so a placeholder or an unresolved secret reference leaves the
provider's status alone. A network problem never causes this message either; if T3 Code cannot
reach Anthropic at all, the provider keeps whatever status it already had.
